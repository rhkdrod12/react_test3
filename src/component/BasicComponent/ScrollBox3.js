...
const useScroll2 = () => {
  ...
  const data = useScrollData();
};

const useScrollData = ({ rowData = [], scrollTop, itemHeight = 50, visibleCount = 5, offsetCnt = 1 }) => {
  // 현재 스크롤에서 시작 idx
  const startIdx = Math.max(Math.floor(scrollTop / itemHeight), 0);
  // 현재 스크롤에서 마지막 idx
  const endIdx = startIdx + visibleCount + offsetCnt;

  return rowData.filter((item, idx) => idx >= startIdx && idx < endIdx);
};
 ...
 * @returns
 */
const ScrollBox = ({ children, itemCount = 0, itemHeight = 50, visibleCount = 5, offsetCnt = 1, scrollWidth = 10, scrollDisplay = true }) => {
  // 스크롤 이벤트 감지 훅 : 스크롤이 이동되면 현재의 스크롤 위치를 반환한다., ref는 스크롤를 감지할 영역
  const [scrollTop, ref] = useScroll(0);
  // 표기해야할 아이템 총개수
  const itemTotalCount = itemCount;
  // 최대 높이(스크롤의 길이)
  ...
  console.log("render Scroll ", `${scrollTop}`);

  return (
    <React.Fragment>
      <StyleOverflowDiv className="scroll-container" height={containerHeight} scrollWidth={scrollWidth} ref={ref}>
        <StyleScrollDiv height={totalHeight}>
          <StyleDiv inStyle={{ transform: `translateY(${offsetY}px)` }}>{children}</StyleDiv>
        </StyleScrollDiv>
      </StyleOverflowDiv>
    </React.Fragment>
  );
